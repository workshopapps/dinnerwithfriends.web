...
export const EmailInvite = () => {
  ...
  const startDate = eventData
    ? moment(eventData?.event?.start_date, "DD-MM-YYYY").format(
        "YYYY-MM-DDTHH:mm"
      )
    : null;
  const endDate = eventData
    ? moment(eventData?.event?.end_date, "DD-MM-YYYY").format(
        "YYYY-MM-DDTHH:mm"
      )
    : null;
  const emailRegex =
  ...
  const declineInvite = () => {
    ...
    setResultMsg(
      "You have succesfully declined this invite... Redirecting to your homepage soon"
    );
    setTimeout(() => {
      navigate("/");
    }, 2000);
  ...
  return (
    <div>
      {resultMsg === "jwt expired" ? (
        <p className="text-red-900 text-4xl font-bold">
          Email invite has expired. Please contact the host for more info.
        </p>
      ) : (
        <>
          <div className="mt-28 mb-10 mx-auto md:mx-32">
            <div className="mx-2 md:mx-0 text-center w-full">
              ...
              <p className="leading-6 text-gray-600 font-sm">
                ...
                {eventData ? eventData?.event?.event_type : ""} by{" "}
                <span className="text-blue-700 font-bold">
                  {" "}
                  {eventData ? eventData?.event?.host_info[0].name : ""}
                </span>
            ...
            <div className="my-8 border py-5 rounded-lg flex justify-center items-center ">
              <form onSubmit={addParticipant}>
                ...
                <div className="my-4 grid">
                  ...
                  <input
                    ...
                    className={
                      emailRegex.test(inviteDetails.email) ||
                      inviteDetails.email === ""
                        ? "outline-none border border-[#898989] bg-transparent rounded md:w-[477px] w-full px-3 py-3 text-base font-medium"
                        : "border border-red-600 bg-transparent rounded outline-none md:w-[477px] w-full px-3 py-3 text-base font-medium"
                    }
                    required
                    autoComplete="true"
                  />
                ...
                {eventData?.event?.final_event_date ? (
                  <p className="text-blue-500 font-semibold text-center w-[200px] md:w-[450px]">
