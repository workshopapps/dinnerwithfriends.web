...
/* global gapi */
const UpcomingEvent = () => {
  const [status, setStatus, loading, setLoading] = useState("not-decided");
  ...
  const filteredEvents =
    events && events.length === 0
      ? events && events
      : events && events.filter((event) => event.published === status);
  ...
  return (
    <>
      ...
      <section className="px-[22px] lg:px-20 pt-[6rem] lg:pt-[7rem] mb-16">
        ...
        <div
          className={`${
            events && filteredEvents.length === 0 ? "pt-12" : "pt-[10px]"
